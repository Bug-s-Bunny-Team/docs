...
%Struttura documento analisi dei requissiti
...
\paragraph*{AWS}
\begin{itemize}
	\item \emph{Fargate}: servizio serverless per la gestione di container
	\item \emph{RDS}: database relazionale in modalità serverless
	\item \emph{Lambda}: servizio di calcolo serverless, basato sugli eventi
	\item \emph{API Gateway}: servizio gestito che semplifica creazione, pubblicazione, manutenzione, monitoraggio e protezione delle API dei vari microservizi sviluppati
	\item \emph{Rekognition}: servizio per l'estrazione di informazioni da immagini e video tramite tecniche di Machine Learning
	\item \emph{Comprehend}: servizio per l'estrazione di informazione da testi, tramite tecniche di Natural Language Processing
	\item \emph{Cognito}: servizio che permette di aggiungere strumenti di registrazione degli utenti, accesso e controllo degli accessi 
\end{itemize}
\paragraph*{Linguaggi di programmazione}
% al momento messi come placeholder, ma probabilmente useremo soltanto questi
\begin{itemize}
	\item \emph{Python}
	\item \emph{Javascript / Typescript}
\end{itemize}
\paragraph*{Librerie e framework}
% al momento non si sa cosa andremo ad usare effettivamente
\begin{itemize}
	\item \emph{React}: libreria per lo sviluppo web frontend
\end{itemize}
