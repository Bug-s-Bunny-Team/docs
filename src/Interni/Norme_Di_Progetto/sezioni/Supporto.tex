		...
		\paragraph{Verbali} 
			...
			La struttura sarà quindi la seguente:
				\begin{itemize}
					...
					\item \textbf{Agenda:} contiene la motivazione per cui si è deciso di fissare la riunione;
					\item \textbf{Resoconto della riunione:} riporta l'esito della discussione dei singoli argomenti trattati;
					\item \textbf{Issue aperte:} vengono elencate le issue\G{} che sono state aperte su Github\G{} durante il verbale.		
				\end{itemize}
	...
	\subsubsection{Versionamento}
		...
		\aCapo{}
			...
			\begin{itemize}
				...
				\item \href{https://github.com/Bug-s-Bunny-Team/bunnyfood}{\textbf{bunnyfood}: per la gestione della piattaforma e del versionamento del codice}.
			\end{itemize}
		...
		\aCapo{}  
			...
			\end{itemize}
			Il \emph{repository\G{}} inerente alla realizzazione della piattaforma sarà composto da diversi \textit{branch\G}:
			\begin{itemize}
				\item \verb#master#:\emph{branch}\G{} principale che viene aggiornato quando la piattaforma è realizzata o per introdurre nuove funzionalità; 
				\item \emph{frontend}\G{}: \emph{branch}\G{} è stato utilizzato il framework\G{} Svelte per la realizzazione del Frontend\G{};
				\item \emph{dev}\G{}: \emph{branch}\G{} è stato utilizzato il linguaggio Python\G{}.
			\end{itemize}

		
		\paragraph{Modifiche alla repository del codice}
		\aCapo{}  
		...
		\paragraph{Modifiche alla repository per la realizzazione della piattaforma}
		\aCapo{}  
		Tutti i membri del gruppo possono apportare modifiche ai file elaborati, salvo quelli presenti nel ramo \textit{master} \\
		Il ramo \textit{master} verrà aggiornato alle ultime modifiche tramite merge dal ramo \textit{dev} prima del rilascio di una versione finale.\\
		Mentre il ramo \textit{dev} verrà aggiornato con le ultime modifiche tramite merge, dall'altro ramo di lavoro. \\
		...
		\subsection{Gestione della qualità}
			...
			\subsubsection{Controllo della qualità}
				...
				\begin{itemize}
					\item Gli obiettivi devono essere compresi, in caso di dubbi bisogna chiarirli con gli altri componenti;
					\item Trovare dei possibili errori, proponendo una soluzione;
					\item Riuscire a stimare la complessità di una task, organizzandosi per terminarla;
					\item Cercare di migliorare progressivamente la conoscenza dell'argomento;
					\item Rispettare le regole che vengono adotatte dal team;
					\item Produrre i risultati desiderati e concreti. 
				\end{itemize}
		...
		\subsection{Verifica}
			...
			\subsubsection{Verifica del codice}
				Il codice, derivante dai requisiti da soddisfare e dalla progettazione effetuata, verrà vericato tramite test automatici. Inoltre sono presenti all'interno del \PdQ{} le metriche e varie misurazioni.  
				...
				correttore automatico per i documenti \LaTeX fornito dalle estensioni di VSC\G{}, che individua le parole non corrette grammaticalmente con una classica sottolineatura.
