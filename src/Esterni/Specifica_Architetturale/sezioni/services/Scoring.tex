...
\end{lstlisting}
Prende in input due file JSON (\verb+textResult+ e \verb+faceResult+), fa lo scoring dei volti 
ed estrapola 
...
la rotazione della testa sull'asse perpendicolare al terrenno, \verb+Pitch+ invece dell'asse parallelo al 
...
La parte riguardante l'emozione \verb+DISGUSTED+ è stata inserita perchè ci è sembrato limitante usare
...
Rispetto al JSON di esempio di cui sopra, il risultato di \verb+sPost.texts+ sarebbe il seguente:
\begin{lstlisting}[language=JSON]
{
    0: 'PIZZA LAB', 
    1: 'Buon Ferragasta'
...
if idx == 0:
    ...
    sPost.textsScore[idx - 1] = float_score
\end{lstlisting}
E lo score che viene calcolato per \verb+idx==0+ del risultato di Comprehend è proprio lo score della
...
\end{lstlisting}
ciò che interessa a noi è la variabile \verb+TextList+, ritornata dalla funzione \verb+__unpack+ 
(\verb+LanguageCode+ infatti è semplicemente una stringa che indica la lingua dominante della caption).
\verb+TextList+ infatti contiene sia la caption che la lista di testi rilevati a schermo, uniti in un
unica variabile dalla funzione \verb+__unpack+:
\begin{lstlisting}[language=Python]
def __unpack_post_for_comprehend(self, sPost: ScoringPost):
...
il valore di sentiment \verb+"Mixed"+ è troppo elevato (questa accortezza serve a filtrare almeno in parte
...
È altamente improbabile che un utente normale usi del testo a immagine per veicolare qualcosa,
...
\end{lstlisting}
salviamo in \verb+textScore+ la media di tutti gli score dei pezzi di testo analizzati, e otteniamo
...
sempre un \verb+finalScore+ compreso tra 0 e 5: se c'è solo uno dei tre score, il suo punteggio acqusisce
peso pari a 5 diventando di fatto il \verb+finalScore+; se manca uno dei tre score gli altri due vengono
pesati a modo per ottenere il risultato finale, sempre tenendo gli scores di volti e caption con peso
uguale e lo score per il testo a schermo con peso minore.
