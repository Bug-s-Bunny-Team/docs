...
Questa sezione  espone il piano di testing che garantisce la correttezza finale del prodotto.
Come definito nelle Norme di Progetto ,il piano segue il modello a V,
il quale ad ogni fase di sviluppo è associato ad  un diverso tipo di testing.
Questi test sono suddivisi in:

\begin{itemize}
	\item \textbf{Test di unità}:viene effettuato per verificare che il comportamento di ogni singola unità sia corretto. Un'unità rappresenta un elemento atomico ed indipendente del sistema.
	\item \textbf{Test di integrazione}:viene effettuato per verificare  il corretto funzionamento di più unità cooperanti che svolgono uno specifico compito.
	\item \textbf{Test di sistema}:viene effettuato per verificare che l’intero sistema funzioni correttamente.I vincoli identificati nell'Analisi dei Requisiti devono essere tutti rispettati.
	\item \textbf{Test di accettazione}:viene effettuato  insieme al  committente durante la fase di collaudo e il loro superamento permette di procedere con il rilascio del prodotto. 
\end{itemize}
...
I test di sistema dimostrano la totale copertura dei requisiti identificati nel documento Analisi dei Requisiti.
{
    \setlength{\freewidth}{\dimexpr\textwidth-10\tabcolsep}
    \renewcommand{\arraystretch}{1.5}
    \centering
