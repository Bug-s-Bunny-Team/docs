...
Questa sezione riporta i test di unità pianificati e/o implementati nei vari componenti dell'applicazione
    ...
    Qui vengono riportati i test di unità della componente Frontend dell'applicazione

    {
        ...
        TUF03.2 & Verificare che la funzione LocationPresenter.getInfo inizializzi correttamente un oggetto Info all'interno di LocationPresenter.info 
                formattato nel modo giusto nel caso che le api siano state caricate in tempo, altrimenti ritorna un errore di timeout & Superato \\
...
\subsection{Test di integrazione}
...

    {
        ...
        TI01 & Verificare che l'integrazione tra la componente Frontend e la pagina di login/registrazione gestita da Cognito (e viceversa) 
            sia gestita in modo corretto & Non implementato \\

        TI02 & Verificare che l'integrazione tra le classi in frontend/src/models/ e i relativi endpoint dell'API backend siano
            gestiti in modo corretto, gestendo tutte le risposte possibili del backend & Non implementato \\
